...
class Accumulator:
    def light_novel(self):
        ...
        try:
            ...
            print("ERROR ENTeRING LIGHT NOVEL")
            print(e)
    ...
    def download_loop(self):
        try:
            ...
            for i in range(number_chapter_downloded -1 ,-1,-1):
                range_bar += range_bar_unit
                self.downloader(self.chapters[i]["link"],"CHAPTER-" + str(self.chapters[i]["number"] + 1),self.chapters[i]["number"] + 1)
                current_message = str(self.chapters[i]["number"] + 1) + "/" + str(number_chapter_downloded)
    ...
    def get_chapter(self):
        ...
        try:
            ...
            chapters_page = requests.get(self.novel_url)
            soup = BeautifulSoup(chapters_page.content, 'html5lib')
            pages = soup.select('.listpage select option')
            
            novel_name = soup.select(".cover>div>.block_txt2>h2")[0].text
            ...
            number_chapter = 0
            for page in pages:
                page = requests.get("https://m.feiszw.com" + page['value'])
                soup = BeautifulSoup(page.content, 'html5lib')
                results = soup.select('.chapter:not(#clist) li')
                for i in range(len(results)):
                    link = results[i].a
                    novel_chapters.append({
                        "number":number_chapter,
                        "link":"https://m.feiszw.com" + link['href']
                    })
                    number_chapter += 1


            #MAKE A DICTIONARY FOR CHAPTER HREF AND NAME
            novel_chapters.reverse()
    ...
    def downloader(self,url,chapter_name,current_loop):
        try:
            try:
                ...
                time.sleep(5)
                print('error occured in link  :'+str(url))
                page=requests.get(url)
            soup = BeautifulSoup(page.content.decode('utf-8'), 'html5lib')
            for s in soup.select('#nr1 span'):
                s.extract()
            results = soup.select('#nr1 p')
            if self.single_file == "y":
                ...
                if self.type == "html":
                    if current_loop == 0:
                        navigation = '<div id="navigation"><a href="#"><button>PREV</button></a><a href="CHAPTER-' + str(current_loop + 1) + '.html"><button>NEXT</button></a></div>'
                    elif current_loop == len(self.chapters):
                        navigation = '<div id="navigation"><a href="CHAPTER-' + str(current_loop - 1) + '.html"><button>PREV</button></a><a href="#"><button>NEXT</button></a></div>'
                    else:
                        navigation = '<div id="navigation"><a href="CHAPTER-' + str(current_loop - 1) + '.html"><button>PREV</button></a><a href="CHAPTER-' + str(current_loop + 1) + '.html"><button>NEXT</button></a></div>'
